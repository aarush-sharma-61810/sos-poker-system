...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (game.countPlayersWithChips() > 1) {
            System.out.println("\n--- HAND " + handNum + " ---");
            game.startHand();
            System.out.println("Dealer: " + game.players.get(game.dealerIndex).name);
            ...
            if (game.getActiveCount() == 1) {
                ...
                System.out.println(winner.name + " wins $" + game.pot + " (all others folded)");
            } else {
    ...
    static void runBettingRound(Game game, String roundName) {
        System.out.println("\n[" + roundName + "]");
    ...
    static void runBetting(Game game, int startIndex) {
        // mark who has already acted this round (folded or broke people skip)
        boolean[] acted = new boolean[game.players.size()];
        for (int i = 0; i < acted.length; i++) {
            Player p = game.players.get(i);
            acted[i] = p.folded || p.chips <= 0;
        }
            ...
            if (!p.folded && p.chips > 0) {
                ...
                if (!acted[index] || needToCall > 0) {
                    ...
                    if (action.equals("f")) {
                        ...
                        if (needToCall > 0) {
                            int amount = Math.min(needToCall, p.chips);
                            p.chips -= amount;
                            p.roundBet += amount;
                            game.addToPot(amount);
                            System.out.println(p.name + " calls $" + amount + ".");
                        } else {
                        ...
                        int minRaise = Math.max(1, game.currentBet - p.roundBet + 1);
                        int raise = askInt("Raise by $", minRaise, p.chips);
                        p.chips -= raise;
                        p.roundBet += raise;
                        game.addToPot(raise);
                        if (p.roundBet > game.currentBet) {
                            game.currentBet = p.roundBet;
                        }
                        System.out.println(p.name + " raises by $" + raise + ".");
                        // when someone raises, reset everyone else so they can respond
                        for (int j = 0; j < acted.length; j++) {
                            if (j != index) {
                                Player other = game.players.get(j);
                                acted[j] = other.folded || other.chips <= 0;
                            }
                        }
                        acted[index] = true;
                    }
    ...
    static boolean allPlayersDone(Game game, boolean[] acted) {
    ...
    static boolean isValidAction(String action, boolean canCheck) {
    ...
    static void pickWinner(Game game) {
        ...
        while (true) {
            ...
            if (winnerName.equalsIgnoreCase("S")) {
                ...
                int share = game.pot / active.size();
                int extra = game.pot % active.size();
                for (int i = 0; i < active.size(); i++) {
                    Player p = active.get(i);
                    p.chips += share + (i == 0 ? extra : 0);
                }
            ...
            for (Player p : active) {
                if (p.name.equalsIgnoreCase(winnerName)) {
                    p.chips += game.pot;
                    System.out.println(p.name + " wins $" + game.pot + ".");
                    return;
                }
            }
    ...
    static void printStacks(Game game) {
        System.out.println("Current stacks:");
        for (Player p : game.players) {
            System.out.println(p.name + ": $" + p.chips);
        }
    }
}
